...
<div class="col-sm-12 col-md-12 col-xl-6">
    <div class="card draggable shadow-sm  {{$bgCard}}" style="height: 270px;{{$styleInline }}">
        <div class="card-body p-2">
            ...
            <small class="{{$texto}}">Demandante: {{$order->customer_name}}</small><br>
            <small class="{{$texto}}">{{ __($order->status) }}</small>
            @if ($order->status === 'reserved' || $order->status === 'in progress')
                <br><small class="{{$texto}}">Responsável: {{$order->provider_name}}</small>
            @endif
            <br><small class="{{$texto}}">Aberto em {{date("d/m/Y G:i:s", strtotime($order->created_at))}} </small>
