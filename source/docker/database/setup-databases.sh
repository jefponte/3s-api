...
function atribui_privilegios() {
    ...
    psql -v ON_ERROR_STOP=1 -d $connection_string_root_con <<-EOSQL
        ...
        ALTER DEFAULT PRIVILEGES IN DATABASE "$database" GRANT USAGE, CREATE ON TABLES TO $user;
        ALTER DEFAULT PRIVILEGES IN DATABASE "$database" GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO $user;
        ALTER DEFAULT PRIVILEGES IN DATABASE "$database" GRANT TRUNCATE, REFERENCES, TRIGGER ON TABLES TO $user;
